...
Base.log(x::Dual{T}) where {T<:Number} = Dual{T}(log(x.a), x.b / (x.a * ), x.b / (x.a * log(x.a))) 
...
function Base.display(d::Dual{T})::String where {T} 
    res = "$(d.a) + $(d.b)ε" 
    println(res) 
    return res 
end 
...
function differentiate_approx(f::Function, x::Number) 
    df = function(y) 
        eps = 1e-5 
